...
def get_max_mz(scan,mz_min,mz_max):
    sub_peaks = list(filter(lambda x: x[0] >= mz_min and x[0] <= mz_max,scan.peaks))
    if len(sub_peaks) == 0:
        return 0.0,-1
    else:
...
def create_plot(lipid_name,output_dict,output_filename = None):
    ...
    plt.plot(t,data_mat[:,0],'ro')
    plt.plot(t,a0 + (ai-a0)*(1-np.exp(-k*t)))
    plt.title('{}  k: {:.3f}, a0: {:.3f}, ai: {:.3f}'.format(lipid_name,k,a0,ai))

    plt_name = '{}_condition_{}_{}.png'.format('_'.join(lipid_name.split()),1,'Pos').replace(':','_')
...
def create_xlsx_output(output_dict,output_filename = 'test.xlsx'):
    ...
    for lipid_no,lipid in enumerate(output_dict):
        ...
        row_num += 1
        write_xlsx_block(worksheet,['isotope details','file','iso number','theoretical m/z','intensity','mz','rt','scan number'],row_num,1)
        row_num += 1
        ...
        create_plot(lipid,output_dict[lipid],output_filename = 'temp_{}.png'.format(lipid_no))
